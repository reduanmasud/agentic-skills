...
The checklists below are **starting points** — generate additional PR-specific test cases. For each changed file/feature, create test cases for: happy path, error path, edge cases, role variations, and regression of related features. See Phase 1 in SKILL.md for minimum journey counts.

### Business Logic Validation (BLV) Test Cases

BLV findings are generated by the **BLV + Security agent in Phase 0** of the main workflow (SKILL.md). They test expected domain behavior **independent of the implementation** — verifying what the feature SHOULD do, not what the code says it does. This catches logic flaws where the implementation's algorithm or approach is fundamentally wrong even though the code runs correctly.

- Stored in `qa-test-progress.json` under `blv_findings`
...
- When a BLV finding is confirmed by testing, report it as a **Logic Flaw** in Section 5.5 of the report, not as a regular bug
- BLV findings inform journey variants and appear in the Phase 6 gap check

See SKILL.md Phase 0.3 (BLV + Security agent) for the complete methodology.
...
**xCloud-specific:** Test the pages and features that share code with the PR's changes (identified in Phase 0 cross-feature consumer analysis).
